...
void KMfilterCenters::computeDistortion() // compute distortions
{
	...
	// below for DB-index & XB-index
	double tmp0, si, sj, sisj, Sijmax, dij, dijmin, Rij;
	double DBIndex = 0, dmin = FLT_MAX;
	for(int i=0; i<kCtrs; i++) 
	{
		sisj = 0;
		Sijmax = 0;
		dijmin = FLT_MAX;
		si = (dists[i]/ weights[i]);
		...
		for(int j=0; j<kCtrs; j++)
		{
			...
			sj = (dists[j] / weights[j]);

			dij = 0;
			for(int d = 0; d < getDim(); d++) 
			{
				tmp0 = (ctrs[i][d] - ctrs[j][d]);
				dij += tmp0 * tmp0;
			}
		...
		if( dijmin < FLT_EPSILON )
			dijmin = FLT_EPSILON;
		if( sisj < 0 )
			sisj = 0;
		Sijmax = (Sijmax + 2 * sqrt(sisj));
		if( Sijmax < FLT_EPSILON)
			Sijmax = FLT_EPSILON;
		Rij = sqrt(Sijmax/dijmin);

		// Punish for very small clusters
		double ratio = (1.0 * weights[i]) / kCtrs;		
		if( ratio < 1.0 || weights[i] < 50 )
		{
			// punish for single samples
			if( weights[i] == 1 )
				Rij *= 100;
			else
			{
				if( ratio < 0.1 && weights[i] < 10 )
					Rij *= 5;
				else if( ratio >= 0.1 && ratio < 0.25 && weights[i] < 50 )
					Rij *= 2;
			}
		}
		DBIndex += Rij;
	}
	currDBIndex = DBIndex/kCtrs;
	currXBIndex = currDist/( getNPts() * dmin);
