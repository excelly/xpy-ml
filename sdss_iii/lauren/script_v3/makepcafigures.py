...
nameList = open(filename).readline().split(',')
names    = dict(zip(nameList, range(len(nameList))))

result = np.loadtxt(filename, skiprows=1,delimiter=',', \
                    dtype={'names'  : nameList, \
                           'formats':('i4','i4','i4','i4','i4','i4','i4','i4','f8','f8','f8')})

mjd   = result['MJD']  
plate = result['Plate']
fiber = result['Fiber']
recon = result['Score: Reconstruction Error']
accum = result['Score: Accumulative Error'] 
dist  = result['Score: M. Distance\n']


sort = np.argsort(dist)[::-1]
wanted_number_of_plots = 100.

if len(mjd) < wanted_number_of_plots:

	number_of_plots = len(mjd)
else:
	number_of_plots = wanted_number_of_plots
...
for i in range(0, number_of_plots):
	...
	plotfilename = 'Figure-' + str(plate[index]) + '-' + str(mjd[index]) + '-' + str(fiber[index]) + '.png'
